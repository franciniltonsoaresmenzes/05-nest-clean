...
describe('Fetch Recent Questions', () => {
  ...
  it('should be able to fetch recent questions', async () => {
    await inMemoryQuestionsRepository.create(
      makeQuestion({ createdAd: new Date(2022, 0, 20) }),
    )
    await inMemoryQuestionsRepository.create(
      makeQuestion({ createdAd: new Date(2022, 0, 18) }),
    )
    await inMemoryQuestionsRepository.create(
      makeQuestion({ createdAd: new Date(2022, 0, 23) }),
    )

    const result = await sut.execute({ page: 1 })

    expect(result.value?.questions).toEqual([
      expect.objectContaining({ createdAd: new Date(2022, 0, 23) }),
      expect.objectContaining({ createdAd: new Date(2022, 0, 20) }),
      expect.objectContaining({ createdAd: new Date(2022, 0, 18) }),
    ])
  })
